...
import 'package:PiliPlus/tv/utils/tv_navigation_registry.dart';
...
class _TvMainState extends State<TvMain> {
  late final List<_TvTab> _tabs = [
    ...
    for (final type in TvPref.tvNavigationBar)
      _TvTab(
        label: type.label,
        builder: (_) => TvNavigationRegistry.build(type),
      ),
  ...
  // Tabs are built (and their controllers created) on first visit only, then kept alive by the IndexedStack, so feeds don't all load at startup.
  final Set<int> _builtTabs = {1};
