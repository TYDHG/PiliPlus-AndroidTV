...
class _TvVideoPageState extends State<TvVideoPage> {
  ...
  static const int _commitEndGuardMs =
      1000; // never commit into the last second
  ...
  void _scrubBy({required bool forward, required bool isRepeat}) {
    ...
    if (!_scrubbing.value) {
      ...
      _scrubOriginMs =
          _pendingLandMs ??
          playerCtr.videoPlayerController!.state.position.inMilliseconds;
    ...
    _scrubTargetMs.value = (_scrubTargetMs.value + deltaMs)
        .clamp(0, durMs)
        .toInt();
  ...
  void _commitScrub() {
    _scrubCommitTimer?.cancel();
    if (!_scrubbing.value)
      return; // already committed/cancelled (OK-spam guard)
    ...
    final seekFuture = playerCtr.seekTo(
  ...
  void _cancelScrubInternal() {
    ...
    final int realSec =
        playerCtr.videoPlayerController?.state.position.inSeconds ??
        _scrubOriginMs ~/ 1000;
  ...
  void _warmVideoShot() {
    if (videoDetailController.isFileSource || !playerCtr.showSeekPreview)
      return;
  ...
  Widget _buildScrubBubble() {
    ...
    if (playerCtr.videoShot case Success(:final VideoShotData response)
        when response.index.isNotEmpty &&
            ...
            response.imgXLen > 0 &&
            response.imgYLen > 0) {
      final int idx = math.max(
        0,
        response.index.where((t) => t <= targetSec).length - 1,
      );
      final int page = (idx ~/ response.totalPerImage).clamp(
        0,
        response.image.length - 1,
      );
                  ...
                  Obx(
                    () => _optionsVisible.value
                        ? TvPlayerOptions(
                            ...
                            initialFocusIndex: _optionsInitialFocusIndex,
                  ...
                  Obx(
                    () => _commentsVisible.value
                        ? TvCommentsPanel(
                            ...
                            replyType:
                                videoDetailController.videoType.replyType,
                            upMid: _upMid,
                            onClose: _closeOptions,
                          )
                        : const SizedBox.shrink(),
  ...
  String get _formatReadout {
    ...
    if (player != null && player.state.width > 0 && player.state.height > 0) {
      buffer.write(' · ${player.state.width}×${player.state.height}');
    }
          ...
          children: [
            ...
            Obx(() {
              ...
              final double bufferedFactor = duration > 0
                  ...
                  : 0.0;
              final double targetFrac = durMs > 0
                  ? (targetMs / durMs).clamp(0.0, 1.0)
                  : 0.0;
              final double originFrac = durMs > 0
                  ? (_scrubOriginMs / durMs).clamp(0.0, 1.0)
                  : 0.0;
              const double ds = TvTheme.designScale;
