...
class _TvPlayerOptionsState extends State<TvPlayerOptions> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => _focusInitialRow());
  }
  ...
  void _focusInitialRow() {
    if (mounted && _rowCount > 0) {
      _nodes[widget.initialFocusIndex.clamp(0, _rowCount - 1)].requestFocus();
    }
  }
  ...
  void _switchAudioQuality(AudioItem item) {
    ...
    if (videoCtr.currentAudioQa?.code != quality) {
      ...
      final bool isDolby =
          quality == AudioQuality.dolby_30250.code ||
      ...
      if (!widget.plPlayerController.tempPlayerConf && !isDolby) {
        GStorage.setting.put(SettingBoxKey.defaultAudioQa, quality);
      }
    }
    widget.onClose();
  }
      ...
      for (var i = 0; i < formats.length; i++)
        Obx(
          () => TvOptionRow(
            ...
            checked:
                videoCtr.currentVideoQa.value != null &&
